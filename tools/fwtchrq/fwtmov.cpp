...
using namespace fwtmov;

static stsfsm::automat VersionAutomat_;

const char *fwtmov::GetLabel( version__ Version )
{
	switch ( Version ) {
	case v0_1:
		return "0.1";
...
version__ fwtmov::GetVersion( const str::string_ &Pattern )
{
...
qRB
	...
	while ( IRow != qNIL ) {
		...
		if ( Moved || Renamed || Created ) {
			Move.Init();

			Path.Init();
			GetPath( GRow, Ghosts, Path );

			if ( !Created )
				Move.Old = Movings.Names.Append( Path );

			if ( ParentIRow == qNIL )
				qRGnr();

			ParentGRow = Content( ParentIRow )->Dir.GhostRow();

			Path.Init();
			if ( ParentGRow != qNIL ) {
				GetPath( ParentGRow, Ghosts, Path );

				Path.Append( '/' );
				Path.Append( Item.Dir.Name );
			} else
				Path = Item.Path;

			Move.New = Movings.Names.Append( Path );
