/*
	Header for the 'session' module by Claude SIMON (http://zeusw.org/epeios/contact.html).
...
namespace session {
	...
	class sessions_unprotected_
	{
	private:
		Q37_MRMDF( agent::agent___, _A, _Agent );
	...
	public:
		...
		sessions_unprotected_ &operator =( const sessions_unprotected_ &S )
		{
			Sessions = S.Sessions;
			Order = S.Order;
			Ids = S.Ids;
	...
	E_AUTO( sessions_unprotected )

	typedef lck::control___<sessions_unprotected_> sessions___;
