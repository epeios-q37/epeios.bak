...
namespace {
        ...
        str::dString &Hash )
    {
        ...
        while ( !Flow.EndOfFlow() ) {
            ...
            SHAUpdate(&Context, (BYTE *)&C, 1);
        }

        SHAFinal(Digest, &Context);

        Hash.Append((bso::sChar *)Digest, 20);
    }
    ...
    qRB
        cio::COut << "MESSAGE: " << Message << txf::nl << txf::commit;
        Flow.Init(Message);
    ...
    qRB
        ...
        Message.Convert(Buffer);

        SHAInit(&Context);
        SHAUpdate(&Context, (BYTE *)Buffer(), Message.Amount());
        SHAFinal(Digest, &Context);
...
namespace {
    ...
    protected:
        ...
        qRB
            ...
            Response.Init();
            ComputeResponse_(str::wString("dGhlIHNhbXBsZSBub25jZQ=="), Response);
//            ComputeResponse_(str::wString("The quick brown fox jumps over the lazy dog"), Response);
            cio::COut << "RESPONSE: '" << Response << '\'' << txf::nl << txf::commit;
        qRR
        qRT
        qRE
	...
	void Test__(void ) {
        ...
        Message.Init("abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq");
        Hash.Init();
