...
namespace rgstry {
	...
	public:
		...
		const value_ &GetValue(
			...
			row__ Row,
			value_ &Value,
			bso::bool__ *Missing ) const;	// Nota : ne met 'Missing' � 'true' que lorque 'Path' n'existe pas. Si 'Missing' est � 'true', aucune action n'est r�alis�e.
		bso::bool__ GetValue(
			const path_ &Path,
			row__ Row,
			value_ &Value ) const
...
# if 0
	...
	inline row__ Fill(
		xtf::extended_text_iflow__ &XFlow,
		const xpp::criterions___ &Criterions,
		registry_ &Registry	) // Peut �tre = 'qNIL', auquel cas une nouvelle 'registry' est cr�ee dont la racine est stock�e dans ce param�tre.
	{
		row__ Row = qNIL;
	qRH
		xpp::context___ Context;
