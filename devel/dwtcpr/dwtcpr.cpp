	...
	while ( Row != qNIL ) {
		if ( Directories( Rows( Row ) )().Exclusion == dwtbsc::xNo ) {
			...
			if ( Status == sCreated ) {
				...
				// Link.Target = TRows( TRow );	// R�pertoire n'existant pas dans la cible, 'Target' est laiss� � 'qNIL' pour signaler le fait qu'il faut mettre tous les sous-r�pertoires en 'Created'.
				Links.Push( Link );
			}
		}
	...
	while ( ( SRow != qNIL ) && ( TRow != qNIL ) ) {
		...
		case -1:
			if ( Exclusion == dwtbsc::xNo ) {
				...
				Dir().Name = Names.Append( SName() );	// D�j� positionn�.
				Dir().Status = sCreated;
				Modified = true;
		...
		case 1:
			...
			Dir().Name = Names.Append( TName() );	// D�j� positionn�.
			Dir().Status = sRemoved;
			TRow = TRows.Next( TRow );
			Modified = true;
			Directories.Store( Dir, Row );
...
qRB
	...
	while ( ( SRow != qNIL ) && ( TRow != qNIL ) ) {
		...
		case -1:
			if ( Exclusion == dwtbsc::xNo ) {
				File.Init();
				File.Name = Names.Append( SName() );	// D�j� positionn�.
		...
		case 1:
			File.Init();
			File.Name = Names.Append( TName() );	// D�j� positionn�.
...
// Ajoute tous les sous-r�pertoires d'un nouveau r�pertoire.
static void Put_(
	drow__ Parent,
	const dwtftr::drows_ &Rows,
	const dwtftr::directories_ &Directories,
...
qRB
	...
	while ( Links.Amount() ) {
		...
		else // On est dans le cas d'un r�pertoire nouvellement cr�e, et on veut donc ajouter tout son contenu � l'arbre des modifications.
