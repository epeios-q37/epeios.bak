...
namespace dwtght {
		...
		const dwtbsc::ghosts_oddities_ &GO )
	{
		...
		if ( ( Pattern.Amount() > ( GO.Prefix.Amount() + GO.Suffix.Amount() + 2 ) )	// '+2' pour les '.' ajout�s automatiquement.
	...
	// Pour ma gestion du renommage/d�placement de r�pertoires.
	class ghost_
	{
	public:
		struct s {
			grow__ Parent;	// R�f�rence au 'ghost' parent (c'est-�-dire au 'ghost' contenant les donn�es du r�pertoire parent.
			str::string_::s Name;
		} &S_;
		str::string_ Name;
		ghost_( s &S )
	...
	status__ CreateGhost(
		...
		grow__ Parent,
		ghosts_ &Ghosts,
		grow__ &Row );	// Cr�e un 'ghost', et retourne son 'Row' dans 'Ghosts' si effectivement cr�e (ce que l'on sait gr�ce au 'status__' retourn�).
